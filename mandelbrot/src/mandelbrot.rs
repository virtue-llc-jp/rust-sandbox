...
fn test_parse_complex() {
    ...
    assert_eq!(parse_complex("0,1.1"), Some(Complex{re: 0.0, im: 1.1}));
    assert_eq!(parse_complex("0,1.1.1"), None);
    assert_eq!(parse_complex("1.1,0"), Some(Complex{re: 1.1, im: 0.0}));
    assert_eq!(parse_complex("1.1.1,0"), None);
    assert_eq!(parse_complex("1.2,3.4"), Some(Complex{re: 1.2, im: 3.4}));
}
...
fn pixel_to_point(bounds: (usize, usize), pixel: (usize, usize), upper_left: Complex<f64>, lower_right: Complex<f64>) -> Complex<f64> {
    ...
    Complex {
        ...
        // -- pixel.1 increases as we go down, the imaginary compinent increases as we go up.
    }
}
...
/// 
/// The `bounds` argument fives the width and height of the buffer `pixels`,
